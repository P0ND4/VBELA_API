...
@Injectable()
export class RecipeGroupRepository extends RecipeGroupRepositoryEntity {
  ...
  async add(identifier: string, group: Group): Promise<ApiResponse<null>> {
    try {
      ...
      return new ApiResponse(
        ...
        user
          ? 'Grupo de recetas creado exitosamente.'
  ...
  async edit(
    ...
    try {
      ...
      // Edit product group
      user.recipeGroup = user.recipeGroup.map((g) => (g.id === id ? group : g));
      ...
      return new ApiResponse(
        ...
        user
          ? 'Grupo de recetas actualizado exitosamente.'
  ...
  async remove(
    ...
    try {
      ...
      user.recipeGroup = user.recipeGroup.filter(
