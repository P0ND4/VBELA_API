...
@Injectable()
export class ProductGroupRepository extends ProductGroupRepositoryEntity {
  ...
  async add(identifier: string, group: Group): Promise<ApiResponse<null>> {
    try {
      ...
      return new ApiResponse(
        ...
        user
          ? 'Grupo de productos creado exitosamente.'
  ...
  async edit(
    ...
    try {
      ...
      // Edit product group
      user.productGroup = user.productGroup.map((g) =>
        g.id === id ? group : g,
      );
      ...
      return new ApiResponse(
        ...
        user
          ? 'Grupo de productos actualizado exitosamente.'
  ...
  async remove(
    ...
    try {
      ...
      user.productGroup = user.productGroup.filter(
      ...
      user.products = user.products.map((product) => ({
        ...product,
        categories: product.categories.filter((c) => c !== groupID),
        subcategories: product.subcategories.filter((s) => s.category !== groupID),
      }));
